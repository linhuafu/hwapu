			  ...
			  ...)
{
  va_list args;
  va_start (args, msg);
  char *errorMessage = g_strdup_vprintf(msg, args);
  va_end (args);
  // Handle 'nbsp' here
  UT_XML * pXML = reinterpret_cast<UT_XML *>(xmlp);
  pXML->incMinorErrors();
  if(strstr(errorMessage,"'nbsp' not defined") != NULL)
  {
  ...
  else if(strstr(errorMessage,"not defined") != NULL)
  {
      pXML->incRecoveredErrors();
  }
  else
