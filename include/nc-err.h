 ...
 * REVISION:
 *
 * 3) Clean away colors, try not splitting printfs. ------- 2007-12-14 nerochiaro
...
#define EPRINT(fmt, arg...) do { \
		printf("[%d] ERROR: %s--%s: %d\nerrno = %d\n\t" fmt, getpid(), __BASE_FILE__, __FUNCTION__, __LINE__, errno, ##arg); \
		assert(0); } while(0)

#define WPRINT(fmt, arg...) do { \
		printf("[%d] WARNING: %s--%s: %d\n\t" fmt, getpid(), __BASE_FILE__, __FUNCTION__, __LINE__, ##arg); \
		} while(0)

#define MPRINT()  do { \
		void * p; p = malloc(10); \
		printf("[%d] %s--%s: %d\n\tmalloc starts from:##0x%X##\n", getpid(), __BASE_FILE__, __FUNCTION__, __LINE__,(int)p); \
		free(p); } while(0)
...
#define FATAL(fmt, arg...) do { \
		printf("[%d] %s--%s: %d\nerrno = %d\n\t" fmt "FATAL", getpid(), __BASE_FILE__, __FUNCTION__, __LINE__, errno, ##arg); \
		exit(0);} while(0)
