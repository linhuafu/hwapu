...
#ifndef ANIMATEPALETTE
#define	ANIMATEPALETTE	0
#endif
...
int	GdConvertEncoding(const void *istr, MWTEXTFLAGS iflags, int cc, void *ostr,
		MWTEXTFLAGS oflags, int ocap);
void	GdGetTextSize(PMWFONT pfont, const void *str, int len, MWCOORD *pwidth,
...
int	GdGetTextSizeEx(PMWFONT pfont, const void *str, int cc,
		...
		MWCOORD *pheight, MWCOORD *pbase, MWTEXTFLAGS flags);
void	GdText(PSD psd,MWCOORD x,MWCOORD y, const void *str, int len,
		MWTEXTFLAGS flags);
...
#define MWHOTPLUG_DEV_SR0		2
#define MWHOTPLUG_DEV_BATTERY	3
#define MWHOTPLUG_DEV_VBUS		4
#define MWHOTPLUG_DEV_ACIN		5
#define MWHOTPLUG_DEV_UDC		6

	int action;
#define MWHOTPLUG_ACTION_ADD		0
#define MWHOTPLUG_ACTION_REMOVE		1
#define MWHOTPLUG_ACTION_CHANGE		2
#define MWHOTPLUG_ACTION_MOVE		3
#define MWHOTPLUG_ACTION_ONLINE		4
#define MWHOTPLUG_ACTION_OFFLINE	5
} hotplug_msg_t;

int  GdOpenHotplug(void);
void GdCloseHotplug(void);
int  GdReadHotplug(hotplug_msg_t *buf);

/* devevdev.c */
int  GdOpenEvdev(int index);
void GdCloseEvdev(int fd);
int  GdReadEvdev(int fd, struct input_event *event);
...
void	GdDrawImageToFit_Animation(PSD psd, MWCOORD x, MWCOORD y, MWCOORD width,
		MWCOORD height, int id, int frame);
MWTIMEOUT	GdGetImageDelayTime(int id, int frame);
void    GdSetTransColor(int id, long transcolor);
int		GdSnapToImage(PSD psd, int x, int y, int width,int height,const char* path);
...
typedef struct {  /* structure for reading images from buffer   */
	...
	unsigned long flag;
#define BUFFER_MALLOC 0x00000001UL
#define BUFFER_MMAP   0x00000002UL
	int (*free_priv)(void*);
	void* priv;
} buffer_t;
void	GdImageBufferInit(buffer_t *buffer, void *startdata, int size);
void	GdImageBufferDestroy(buffer_t *buffer);
extern inline unsigned long	GdImageBufferTell(buffer_t *buffer);
extern inline void	GdImageBufferSeekTo(buffer_t *buffer, unsigned long offset);
extern inline void*	GdImageBufferPtr(buffer_t *buffer);
extern inline unsigned long	GdImageBufferRemained(buffer_t *buffer);
int	GdImageBufferRead(buffer_t *buffer, void *dest, unsigned long size);
extern inline int	GdImageBufferGetChar(buffer_t *buffer);
char*	GdImageBufferGetString(buffer_t *buffer, char *dest, unsigned int size);
extern inline int	GdImageBufferEOF(buffer_t *buffer);
...
int GdEncodeJPEG(PSD psd, int x, int y, int width,int height,const char* path);
#endif
...
void		GdRestartTimer(MWTIMER *timer);
