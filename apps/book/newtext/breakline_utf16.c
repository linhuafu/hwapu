...
/*
utf16的breakline
 *注意:utf16暂时只处理UCS-2编码集即编码范围0x0000-0xffff
 *            4B编码范围如以后有特殊要求再加
 *
 *
 *函数set_linebreaks_utf16的用法注意:
 *第4个参数brks的返回值是以字符为单位的，且逗号所在位置为nobreak
 *如Are you
 * brks[0] = no,brks[1] = no,brks[2] = no,brks[3] = Allow,brks[4] = no,brks[5] = no,brks[6] = must(最后一个肯定为must)
 *        A                r                e                 blank                y               o           u
 */
					...
					const char *word, size_t len, GR_SIZE* width)
{
    ...
    while (low < high)
	{
		...
		*width = w;
		...
		{
			int next = utf16_next_nchar(word, mid, 1);//找到下一个字符的首编码
			if (next >= high)
				break;
		...
		const char *text, size_t len, int max_line,int line_from[],int line_len[],int line_width[])
{
	...
	int i;
	int line_start;//每行行首所在(相对于整个字符串)，单位为word
	int last_word;//上一次字符串宽度小于给定宽度时的那个字符所在,单位为word
	int line_count = 0;//可输出行数
	int word_locate;
	int wordcount = len/2;//字符个数
