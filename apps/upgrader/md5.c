...
int main()
{
	...
	rdfd = open(MD5_FILE_NM, O_RDONLY);
	if(-1 == rdfd)
	{
		printf("no such file\n");
		return -1;
	}
	flen = lseek(rdfd, 0, SEEK_END);
	if(-1 == flen)
	{
		printf("seek end err\n");
		iRet = -1;
	...
	printf("---------> filelen: %d\n",flen);
	while(flen >= MD5_MALLOC_SZ)
	{
		iRet = read(rdfd, prd, MD5_MALLOC_SZ);
		printf("IRET:%d\n",iRet);
	...
	if(flen)
	{
		...
		MD5Update(&md5, prd, flen);   //对欲加密的字符进行加密
	}

	MD5Final(decrypt_out,&md5);                                             //获得最终结果
	printf("print ..........\n");

	for(i=0;i<16;i++)
		printf("%2x ",decrypt_out[i]);
	printf("\n");
	printf("over ..........\n");
GENERALERR:
	free(prd);
