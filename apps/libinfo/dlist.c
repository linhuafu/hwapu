...
static void dlist_destroy_data(DList* thiz, void* data)
{
	...
	{
//		printf("dlist_destroy_data data free\n");
...
//	printf("dlist_create_node DListNode malloc\n");
...
static void dlist_destroy_node(DList* thiz, DListNode* node)
{
	if(node != NULL)
	{
		...
		SAFE_FREE(node);
//		printf("dlist_destroy_node DListNode free\n");
	}
...
//	printf("dlist_create dlist malloc\n");
...
Ret dlist_insert(DList* thiz, void* data)
{
	Ret ret = RET_OK;
	DListNode* node = NULL;
	...
	cursor = thiz->end;           //dlist_get_node(thiz, index, 1);  //定位节点
#if 0	
...
Ret dlist_visit(DList* thiz, size_t index, DataVisitFunc visit, void* ctx)
{
	Ret ret = RET_OK;
	...
	return cursor != NULL ? ret : RET_INVALID_PARAMS;

}
Ret dlist_visitpage(DList* thiz, size_t start, size_t page_num, DataVisitFunc visit, void* ctx)
{
	Ret ret = RET_OK;
	DListNode* iter = NULL;
	size_t count = 0;
	...
	iter = dlist_get_node(thiz, start, 0);

	while(iter != NULL  && ret != RET_STOP && count < page_num)
	{
	...
	return count != 0 ? ret : RET_INVALID_PARAMS;
...
Ret dlist_foreach(DList* thiz, DataVisitFunc visit, void* ctx)
{
	...
	while(iter != NULL && ret != RET_STOP)	//遍历数据
	{
		ret = visit(ctx, iter->data);
...
void dlist_destroy(DList* thiz)
{
	...
	thiz->first = NULL;
	thiz->data_destroy = NULL;
	
	SAFE_FREE(thiz);
//	printf("dlist_destroy DList\n");
...
static Ret sum_longlong(void* ctx, void* data)
{
...
static Ret max_val(void* ctx, void* data)
{
    long long* max = ctx;
